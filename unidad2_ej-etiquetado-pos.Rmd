...
Si quisiéramos usar esta información morfosintáctica para un análisis de estilo (como la correlación uso de adverbios y calidad de escritura presentada al inicio de la guía), podemos con filtrar por **upos** usando `filter()`:
...
Leemos el archivo 'DuBiscay_RelDeUnViaje.txt' (disponible [online](https://raw.githubusercontent.com/hdcaicyt/Estilometria-con-R/gh-pages/corpus/DuBiscay_RelDeUnViaje.txt)). **Atención**: si alojaste el texto en otra ubicación, vas a tener que modificar el path que le pasamos a `read_lines()`. Recordá que también podés pasarle una url a `read_lines()`.
...
* Leer los txt de los discursos utilizados la semana pasada
* Etiquetarlos morfosintácticamente con `udpipe_annotate`
* Identificar los sustantivos más frecuentes y mostrarlos en un gráfico de barras
* Buscar las coocurrencias consecutivas de verbo-sustantivo y mostrarlas en un grafo
* Buscar las coocurrencias no consecutivas de adjetivo-sustantivo y mostrarlas en un grafo
